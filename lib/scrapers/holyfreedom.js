 ...
 * - Parse cache_default_attribute (default combo ID) from data-product
 * - Radio inputs are sequential: comboId = defaultComboId + radioIndex - defaultRadioIndex
 * - Fetch each ?id_product_attribute=N URL inside browser (uses session cookies)
 * - Parse data-product from returned HTML to get per-variant EAN/UPC/qty
 */
async function scrapeProduct(page, url) {
  ...
  // Parse data-product JSON (current selected variant)
  let productData = null
  try { productData = JSON.parse($('#product-details, .js-product-details').first().attr('data-product') || '') } catch (_) {}

  const base = { name, reference, brand, price, category, description_short, description, images, url }

  // Collect size radio inputs with index
  const sizeOptions = []
  $('input.input-radio[name^="group"]').each((i, el) => {
    const $el = $(el)
    sizeOptions.push({
      index:     i,
      attrName:  $el.attr('title') || '',
  ...
  if (sizeOptions.length === 0) {
    const attr = productData?.attributes ? Object.values(productData.attributes)[0] : null
    return [{
      ...base,
      taglia:       attr?.name || '',
  ...
  // default combo ID — stored as string in PS JSON, must parseInt
  const defaultComboId = productData?.cache_default_attribute
    ? parseInt(productData.cache_default_attribute)
    : null

  if (!defaultComboId) {
    // Can't determine combo IDs — fall back to single row with base data
    const attr = productData?.attributes ? Object.values(productData.attributes)[0] : null
    return [{
      ...base,
      taglia:       attr?.name || sizeOptions.map(o => o.attrName).join(' / '),
      ean:          String(attr?.ean13 || ld.gtin13 || ld.gtin14 || ld.gtin || ''),
      barcode:      String(attr?.upc || ''),
      availability: baseAvailability,
    }]
  }

  const defaultRadioIndex = sizeOptions.findIndex(o => o.isChecked)
  const refIndex          = defaultRadioIndex >= 0 ? defaultRadioIndex : 0
  const cleanUrl          = url.split('?')[0]

  const variants = []
  for (const opt of sizeOptions) {
    const comboId    = defaultComboId + opt.index - refIndex
    const variantUrl = cleanUrl + '?id_product_attribute=' + comboId

    try {
      const variantHtml = await page.evaluate(async (variantUrl) => {
        try {
          const r = await fetch(variantUrl, { credentials: 'include' })
          return r.ok ? r.text() : null
        } catch (_) { return null }
      }, variantUrl)

      if (!variantHtml) {
        variants.push({ taglia: opt.attrName, ean: '', barcode: '', quantity: -1 })
        continue
      }

      const m = variantHtml.match(/data-product="([^"]+)"/)
      if (!m) {
        variants.push({ taglia: opt.attrName, ean: '', barcode: '', quantity: -1 })
        continue
      }
      ...
      const attr = d.attributes ? Object.values(d.attributes)[0] : {}
      variants.push({
        taglia:   opt.attrName || attr?.name || '',
