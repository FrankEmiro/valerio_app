...
const COLUMN_GROUPS = [
  { id: 'product',  label: 'Prodotto',       icon: '⬡', columns: ['name', 'reference', 'ean', 'barcode', 'brand', 'url'] },
...
async function scrapeProduct(page, url) {
  ...
  // EAN: JSON-LD gtin fields first, then features table
  let ean = ld.gtin13 || ld.gtin14 || ld.gtin8 || ld.gtin || ''
  // Barcode: product features section (PrestaShop .data-sheet)
  let barcode = ''
  $('.data-sheet dt.name, .product-features dt').each((_, el) => {
    const label = $(el).text().trim().toLowerCase()
    const val   = $(el).next('dd').text().trim()
    if (!ean     && (label.includes('ean') || label.includes('gtin'))) ean     = val
    if (!barcode && (label.includes('barcode') || label.includes('a barre') || label.includes('barre'))) barcode = val
  })

  return { name, reference, ean, barcode, brand, price, availability, category, description_short, description, images, url }
}
