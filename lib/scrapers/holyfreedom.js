...
// Non-product slugs to skip when parsing sitemap
const SITEMAP_SKIP = new Set([
  'pagina-non-trovata','piu-venduti','contattaci','marche','nuovi-prodotti',
  'recupero-password','offerte','fornitori','indirizzo','indirizzi','content',
  'authentication','order','cart','account','login','register','checkout',
  'password','guest','brand','ricerca','sconti','segui-ordine','supplier',
  'tracciatura-ospite','cronologia-ordini','dati-personali','conferma-ordine',
  'negozi','ordine','buoni-sconto','buono-ordine','home','carrello',
  'mappa-del-sito',
])
...
async function collectProductUrls(categories, page, log) {
  ...
  function addUrl(raw) {
    const url = String(raw || '').replace(/^http:\/\//, 'https://').trim()
    if (url && !seen.has(url)) { seen.add(url); links.push(url); return true }
    return false
  }

  // ── 1. Sitemap (primary source — covers donna/caschi/accessori that AJAX can't reach) ──
  log('Recupero URL da sitemap...')
  const sitemapXml = await page.evaluate(async (url) => {
    try {
      const r = await fetch(url, { credentials: 'include' })
      return r.ok ? r.text() : null
    } catch (_) { return null }
  }, `${BASE_URL}/1_it_0_sitemap.xml`)

  if (sitemapXml) {
    let sitemapCount = 0
    for (const m of sitemapXml.matchAll(/CDATA\[\s*(https?:\/\/[^\]]+?)\s*\]/g)) {
      const raw = m[1].trim()
      // Skip images and non-/it/ URLs
      if (!raw.includes('/it/') || /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(raw)) continue
      const itPath = raw.split('/it/')[1] || ''
      const parts  = itPath.split('/').filter(Boolean)
      // Need at least category + product-slug depth
      if (parts.length < 2) continue
      if (SITEMAP_SKIP.has(parts[0].toLowerCase())) continue
      // Filter to requested categories (match by prefix)
      const matchesCat = categories.some(cat => {
        const catParts = cat.split('/')
        return catParts.every((seg, i) => parts[i] && parts[i].toLowerCase() === seg.toLowerCase())
      })
      if (!matchesCat) continue
      if (addUrl(raw)) sitemapCount++
    }
    log(`Sitemap: ${sitemapCount} URL trovati`)
  } else {
    log('Sitemap non disponibile — procedo solo con AJAX')
  }

  const beforeAjax = links.length

  // ── 2. AJAX supplement (catches new products not yet indexed in sitemap) ──
  for (const cat of categories) {
    ...
    while (pageNum <= totalPages) {
      ...
      if (data?._error || data?._html !== undefined) break

      const products = data?.products || []
      if (!products.length) break

      if (pageNum === 1) totalPages = data.pagination?.pages_count || 1

      for (const prod of products) {
        const url = String(prod.url || prod.canonical_url || '')
        if (addUrl(url)) catAdded++
      }
    ...
    if (catAdded > 0) log(`  AJAX /${cat}: +${catAdded} nuovi (non in sitemap)`)
    await sleep(500)
  }

  const ajaxNew = links.length - beforeAjax
  if (ajaxNew > 0) log(`AJAX supplement: +${ajaxNew} URL aggiuntivi`)
  log(`Totale URL prodotti: ${links.length}`)
