...
/** Returns an array of rows — one per variant (or one if no variants found). */
async function scrapeProduct(page, url) {
  // Navigate so inline scripts execute and combinations populate window.combinations
  const resp = await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 })
  if (resp && !resp.ok()) throw new Error(`HTTP ${resp.status()}`)

  // Wait up to 2s for PrestaShop to expose combinations (inline or async)
  await page.waitForFunction(
    () => !!(window.combinations || window.prestashop?.product?.combinations || window.productCombinations),
    { timeout: 2000 }
  ).catch(() => {})
  ...
  // Product-level EAN/barcode fallback
  let baseEan = String(ld.gtin13 || ld.gtin14 || ld.gtin8 || ld.gtin || '')
  let baseBarcode = ''
  $('.data-sheet dt.name, .product-features dt').each((_, el) => {
    const label = $(el).text().trim().toLowerCase()
    const val   = $(el).next('dd').text().trim()
    if (!baseEan     && (label.includes('ean') || label.includes('gtin'))) baseEan     = val
    if (!baseBarcode && (label.includes('barcode') || label.includes('a barre') || label.includes('barre'))) baseBarcode = val
  })

  const base = { name, reference, brand, price, category, description_short, description, images, url }

  // Read combinations directly from browser JS context (reliable for all PS setups)
  const rawCombos = await page.evaluate(() => {
    try {
      const c = window.combinations
        || window.prestashop?.product?.combinations
  ...
  // Fallback: HTML-based extraction (handles static embeds missed above)
  const combos = rawCombos ? normalizeCombinations(rawCombos) : extractCombinations($, html)

  if (!combos || combos.length === 0) {
    return [{ ...base, taglia: '', ean: baseEan, barcode: baseBarcode, availability: baseAvailability }]
  }

  return combos.map(combo => ({
    ...base,
    taglia:       combo.taglia,
    ean:          combo.ean     || baseEan,
    barcode:      combo.barcode || baseBarcode,
    availability: combo.quantity === 0 ? 'Non disponibile'
                : combo.quantity  >  0 ? 'Disponibile'
                : baseAvailability,
  }))
}
