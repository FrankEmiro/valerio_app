...
const COLUMNS = [
  ...
  { key: 'price',            header: 'Price GBP (trade)',   width: 18 },
  { key: 'price_eur',        header: 'Prezzo EU (cambio)',  width: 22 },
  { key: 'price_vale',       header: 'Prezzo Vale (+60%+IVA)', width: 22 },
...
function buildPriceVale(priceStr, rate) {
  ...
  const gbp = parseGbpAmount(priceStr)
  if (gbp == null) return ''
  // Prezzo da loggati (trade, no IVA no margine) → +60% margine → +22% IVA italiana
