...
const COLUMNS = [
  ...
  { key: 'price',            header: 'Price GBP',           width: 18 },
  { key: 'price_eur',        header: 'Prezzo EU (cambio)',  width: 22 },
  { key: 'price_vale',       header: 'Prezzo Vale',         width: 20 },
...
function mergeCookies(...cookieHeaderArrays) {
  const jar = new Map()
  for (const arr of cookieHeaderArrays) {
    for (const setCookie of (arr || [])) {
      const [pair] = setCookie.split(';')
      const idx = pair.indexOf('=')
      if (idx === -1) continue
      const name = pair.slice(0, idx).trim()
      const value = pair.slice(idx + 1).trim()
      if (value === 'deleted') jar.delete(name)
...
// ─── Login account trade (per prodotti visibili solo da loggati) ──

async function loginMotone(username, password, log) {
  try {
    const getResp = await axios.get(`${BASE_URL}/login`, { headers: HTTP_HEADERS, timeout: 15000 })
    const baseCookie = mergeCookies(getResp.headers['set-cookie'])

    const body = new URLSearchParams({
      referer: '',
      action: 'customer_login',
      email_address_login: username,
      password_login: password,
    }).toString()

    const postResp = await axios.post(`${BASE_URL}/checkout/register/`, body, {
      headers: {
        ...HTTP_HEADERS,
    ...
    const stillOnLoginForm = $('input[name="email_address_login"]').length > 0
    if (stillOnLoginForm) {
      const errMsg = $('.error, .alert').first().text().trim()
      if (log) log(`Login MotoOne fallito${errMsg ? ' – ' + errMsg : ''}`)
      return null
    }

    const finalCookie = mergeCookies(getResp.headers['set-cookie'], postResp.headers['set-cookie'])
    if (log) log('Login MotoOne riuscito — sessione trade attiva')
    return finalCookie
  } catch (err) {
    if (log) log(`Login MotoOne errore: ${err.message}`)
    return null
  }
}
...
// Prodotti pubblici (prezzo retail, GBP già IVA UK inclusa) → solo margine +8%
...
// Prodotti visibili solo da loggati (prezzo trade netto, no IVA no margine)
// → +60% margine → +22% IVA italiana
function buildPriceValeTrade(priceStr, rate) {
  if (!rate) return ''
  const gbp = parseGbpAmount(priceStr)
  if (gbp == null) return ''
  const eur = Math.round(gbp * rate * 100) / 100
  const withMargin = Math.round(eur * 1.60 * 100) / 100
  const withVat = Math.round(withMargin * 1.22 * 100) / 100
  return withVat
}
  ...
  if (fileLoaded) {
    ...
    if (fileUrlColIdx > 0) {
      sheet.eachRow((row, rowNum) => {
        ...
        if (url) { doneUrls.add(url); urlToRowNum.set(url, rowNum) }
      })
    }
...
async function fetchAjaxData(parentProductId, subProductId, sessionCookie) {
  try {
...
async function scrapeProduct(url, sessionCookie) {
  const resp = await axios.get(url, {
    headers: { ...HTTP_HEADERS, ...(sessionCookie ? { cookie: sessionCookie } : {}) },
    timeout: 15000
  })
  ...
  if (parentProductId && subProductId) {
    const ajax = await fetchAjaxData(parentProductId, subProductId, sessionCookie)
...
async function run(settings, onLog) {
  ...
  let sessionCookie = settings.mo_session_cookie || ''
  const forceUpdate = settings.mo_force_update === 'true'

  if (settings.mo_username && settings.mo_password) {
    log('Login account trade MotoOne...')
    const autoCookie = await loginMotone(settings.mo_username, settings.mo_password, log)
    if (autoCookie) sessionCookie = autoCookie
    else if (sessionCookie) log('Login automatico fallito — uso il Session Cookie manuale salvato nelle impostazioni')
  }
  ...
  // Con force update elimina il file esistente per ripartire da zero (evita duplicati)
  ...
  // Prodotti visibili SOLO da loggati (prezzo trade netto) vs prodotti pubblici
  // (prezzo retail) richiedono due formule di calcolo diverse — li distinguiamo
  // confrontando la scansione con cookie contro una scansione anonima.
  let tradeOnlyUrls = new Set()
  if (sessionCookie) {
    log('Verifico quali prodotti sono visibili solo da loggati...')
    const anonLinks = await collectProductLinks(categoryUrl, maxProducts, childCategories, '')
    const anonSet = new Set(anonLinks)
    tradeOnlyUrls = new Set(allLinks.filter(u => !anonSet.has(u)))
    if (tradeOnlyUrls.size > 0) log(`Prodotti solo-da-loggati trovati: ${tradeOnlyUrls.size}`)
  }

  const toScrape = forceUpdate
    ? allLinks.slice(0, maxProducts)
    : allLinks.slice(0, maxProducts).filter(u => !doneUrls.has(u))
  log(`Trovati: ${allLinks.length} | Già fatti: ${doneUrls.size} | Da fare: ${toScrape.length}${forceUpdate ? ' (aggiornamento forzato)' : ''}`)
  ...
  const appendProduct = (product) => {
    writeQueue = writeQueue.then(async () => {
      ...
      product.price_vale = tradeOnlyUrls.has(product.url)
        ? buildPriceValeTrade(product.price, gbpEurRate)
        : buildPriceVale(product.price, gbpEurRate)
  ...
  async function worker(tasks, startIdx) {
    ...
    while (i < tasks.length) {
      ...
      const num = ++doneCount
      try {
        // Cookie solo per i prodotti da login: da loggati TUTTI i prezzi diventano trade,
        // quindi i prodotti pubblici vanno scrapati da logout per avere il prezzo retail
        const product = await scrapeProduct(url, tradeOnlyUrls.has(url) ? sessionCookie : '')
