...
function buildPriceVale(priceStr, rate) {
  ...
  const gbp = parseGbpAmount(priceStr)
  if (gbp == null) return ''
  return `€${(gbp * rate * 1.08).toFixed(2)}`
}
...
async function scrapeProduct(url) {
  ...
  const price = priceRaw || (priceContent ? `£${priceContent}` : '')
  const shipping = $('#product_shipping_price .inc .GBP').first().text().trim()
