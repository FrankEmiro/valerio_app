...
app.get('/api/status', (req, res) => {
  ...
  res.json({
    sources: {
      ...
      holyfreedom: {
        isRunning: isRunning('holyfreedom'),
        categories: s.hf_categories,
        cronSchedule: s.hf_cron,
        nextRunLabel: CRON_LABELS[s.hf_cron] || s.hf_cron,
        stats: db.getStats('holyfreedom'),
        lastRun: db.getRuns('holyfreedom', 1)[0] || null,
        activeRun: isRunning('holyfreedom') ? db.getActiveRun('holyfreedom') : null,
      }
    },
...
app.delete('/api/runs/:id', (req, res) => {
  db.deleteRun(parseInt(req.params.id))
  res.json({ ok: true })
})
...
app.post('/api/settings', (req, res) => {
  ...
  if (data.hf_cron && _cronTasks.holyfreedom) {
    _cronTasks.holyfreedom.stop()
    app.emit('cron-changed', 'holyfreedom', data.hf_cron)
  }
  res.json({ ok: true })
})
...
app.get('/api/columns/:source', (req, res) => {
  ...
  if (source === 'holyfreedom') {
    let enabled
    try { enabled = JSON.parse(s.hf_columns) } catch (e) { enabled = holyfreedom.ALL_COLUMN_KEYS }
    const enabledSet = new Set(enabled)
    return res.json({
      groups: holyfreedom.COLUMN_GROUPS.map(g => ({
        ...g,
        columns: g.columns.map(key => ({
          key,
          name: holyfreedom.COLUMNS.find(c => c.key === key)?.header || key,
          enabled: enabledSet.has(key)
        }))
      })),
      totalEnabled: enabled.length,
      totalAll: holyfreedom.ALL_COLUMN_KEYS.length,
      keyField: 'key'
    })
  }
...
app.post('/api/run/:source', async (req, res) => {
  const { source } = req.params
  if (!['partseurope', 'motone', 'holyfreedom'].includes(source))
...
app.get('/api/download/:source', async (req, res) => {
  ...
  } else if (source === 'holyfreedom') {
    xlsxPath = path.join(__dirname, '..', 'output', 'holyfreedom', 'products.xlsx')
    filename  = 'holyfreedom_products.xlsx'
    let keys = holyfreedom.ALL_COLUMN_KEYS
    try { keys = JSON.parse(s.hf_columns) } catch (_) {}
    const keyToHeader = Object.fromEntries(holyfreedom.COLUMNS.map(c => [c.key, c.header]))
    enabledHeaders = keys.map(k => keyToHeader[k]).filter(Boolean)
  } else {
    return res.status(404).json({ error: 'Sorgente non valida' })
  }
...
function getEnabledHeaders(source, s) {
  ...
  if (source === 'holyfreedom') {
    ...
    try { keys = JSON.parse(s.hf_columns) } catch (_) {}
    const keyToHeader = Object.fromEntries(holyfreedom.COLUMNS.map(c => [c.key, c.header]))
    return { headers: keys.map(k => keyToHeader[k]).filter(Boolean), brand: 'HolyFreedom', filename: 'holyfreedom_products.xlsx', xlsxPath: path.join(__dirname, '..', 'output', 'holyfreedom', 'products.xlsx') }
  }
  return null
}
...
app.post('/api/send-email/:source', async (req, res) => {
  ...
  if (!hasSmtp && !hasGmail)
    return res.status(400).json({ error: 'Nessun provider email configurato (Gmail o SMTP Brevo)' })
  ...
  try {
    ...
    try {
      await sendExcel({ xlsxPath: tmpPath, brand: info.brand, recipientEmail: recipient, gmailUser: s.gmail_user, gmailAppPassword: s.gmail_app_password, smtpHost: s.smtp_host, smtpPort: s.smtp_port, smtpUser: s.smtp_user, smtpPass: s.smtp_pass })
    } finally {
...
app.post('/api/test-email', async (req, res) => {
  ...
  if (!recipient) return res.status(400).json({ error: 'Nessun destinatario configurato' })
  try {
    const { buildTransport, senderAddress } = require('./mailer')
