...
async function scrapeAll(brand, cookie, enabledColumns = null, onLog = null, forceUpdate = false) {
  ...
  // Con force update azzera il done set e cancella il CSV/xlsx esistente per ripartire da zero
  if (forceUpdate) {
    saveDone(brand, new Set())
    const csvPath = path.join(OUTPUT_DIR, `${brand}.csv`)
    const xlsxPathOld = path.join(OUTPUT_DIR, `${brand}.xlsx`)
    if (fs.existsSync(csvPath)) fs.unlinkSync(csvPath)
    if (fs.existsSync(xlsxPathOld)) fs.unlinkSync(xlsxPathOld)
    log('File precedenti eliminati — riscrapo da zero')
  }
  ...
  log(`Già scrapati: ${done.size}${forceUpdate ? ' (aggiornamento forzato)' : ''}`)
  ...
  for (let i = 0; i < archive.length; i++) {
    ...
    if (!forceUpdate && done.has(url)) continue
